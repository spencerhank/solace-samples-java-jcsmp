 ...
 * It connects to a Solace message broker, serializes a request message using AvroSerializer, sends it to a topic,
 * waits for a reply message and then once received, deserializes the reply message using AvroDeserializer.
 */
public class AvroSerdesRequestor {
    ...
    public static final String REPLY_TOPIC = "solace/samples/create-user-response/avro";
    
    // Timeout for waiting for a reply (in milliseconds)
    private static final int REQUEST_TIMEOUT_MS = 5000;

    /**
     * The main method that demonstrates the Solace JCSMP API usage with Avro serialization and deserialization for Request-Reply.
     *
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length < 3) {
            System.out.printf("Usage: %s <host:port> <message-vpn> <client-username> [password]%n%n", AvroSerdesRequestor.class.getName());
            System.exit(-1);
        }
        ...
        // Create and configure Avro serializer for requests and deserializer for replies
        try (Serializer<GenericRecord> serializer = new AvroSerializer<>();
            ...
            // Create and populate a GenericRecord with sample data for the request
            ...
            while (!exitListener.isDone()) {
                ...
                try {
                    ...
                    GenericRecord userResponse = SerdeMessage.deserialize(deserializer, replyMsg);
