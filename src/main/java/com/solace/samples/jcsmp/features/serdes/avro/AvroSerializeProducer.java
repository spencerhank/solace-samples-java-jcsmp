...
/**
 * SerializeProducer
 * This class demonstrates how to use Solace JCSMP API with Avro serialization to produce messages.
...
public class AvroSerializeProducer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length < 3) {
            System.out.printf("Usage: %s <host:port> <message-vpn> <client-username> [password]%n%n", AvroSerializeProducer.class.getName());
            System.exit(-1);
        }
        ...
        try (Serializer<GenericRecord> serializer = new AvroSerializer<>()) {
            ...
            while (!exitListener.isDone()) {
                //update message
                user.put("id", String.format("%d", index));
    ...
    private static GenericRecord initEmptyUserRecord() throws IOException {
        try (InputStream rawSchema = HelloWorldJCSMPAvroSerde.class.getResourceAsStream("/avro-schema/user.avsc")) {
